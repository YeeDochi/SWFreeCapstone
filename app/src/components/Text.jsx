...
const Text = () => {
  ...
  const handleImageSelect = async (url) => {
    ...
    try {
      const response = await axios.post('http://localhost:8080/api/get_name_by_url', { url }, { withCredentials: true });
  ...
  const handleMessageSend = async () => { //보내깅
    ...
    try {
      ...
      if (response.data.messageKey) {
        ...
        alert('메시지 발송 실패');
        console.log('수신번호 형태:', recipients);
      }
    } catch (error) {
      ...
      <div className="main-text">
        ...
        <div className="recipient-section">
          <div className="recipient-header">
            <h2>📞 수신번호 설정</h2>
            <button onClick={openRecipientPopup} style={{width:"200px"}} className="recipient-button">
              수신번호 추가/편집
            </button>
          </div>
